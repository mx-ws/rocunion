...
void *roc_mmap(void *addr, int length, int prot, int flags, int fd,
               int offset) {
#ifdef _WIN32
  return addr;
#else
...
union MainTypeUnion {
  ...
  struct RocList div_l;
};

struct MainType {
  union MainTypeUnion payload;
  unsigned char discriminant;
};

extern void roc__mainForHost_1_exposed_generic(struct MainType *mainReturn);

int main() {
  struct MainType res;
  roc__mainForHost_1_exposed_generic(&res);

  printf("tag = %d\n", res.discriminant);
  struct RocStr str = res.payload.str;
